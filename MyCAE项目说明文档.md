...
- 基于 VTK 的几何、网格和结果云图显示，并支持网格透明显示控制；
...
-> VtkRenderCanvas 根据显示设置切换实体/半透明网格
```
...
其中 `ResultDisplayController` 负责决定显示字段、标量范围、变形比例、网格边和未变形轮廓等参数；`VtkRenderCanvas` 负责具体 VTK 场景渲染，并提供几何边线、方向坐标轴和网格透明显示等交互显示控制；`ResultPostprocessPanel` 负责在界面中展示后处理控制和结果摘要。
...
近期新增的“网格透明显示”功能进一步增强了网格查看能力。用户可以在菜单中切换网格实体显示和半透明显示：实体显示适合观察模型外形，半透明显示适合观察内部四面体单元、背面轮廓和整体网格空间结构。该功能由 `RenderView` 和 `VtkRenderCanvas` 提供显示状态控制，由 `DisplayModeCommands` 接入菜单命令。
...
该方案的关键点是网格不仅作为显示数据存在，还要保留边界信息，用于后续求解器边界条件映射。同时，网格显示不再是固定样式，用户可以通过“仿真 -> 网格透明显示”在实体显示和半透明显示之间切换，方便在演示和调试时观察不同层次的网格结构。
...
该功能用于控制网格模型的透明度。勾选后网格半透明，便于观察内部单元；取消勾选后网格不透明，便于观察模型外表面。

### 6.6 验证运行

项目提供自动化验证入口，可以检查样例工程、UI 冒烟测试和自动截图采集是否正常。
...
该功能用于展示网格显示样式切换效果。建议正式提交文档时补充两张对比截图：

```text
网格实体显示：用于观察模型外表面和整体轮廓
网格透明显示：用于观察内部四面体单元和背面结构
```

截图时可以打开样例工程，选择已有网格，依次执行：
...
3. 几何模型显示界面；
4. 面拾取和面组创建界面；
5. Gmsh 网格生成后的实体网格显示界面；
6. 网格透明显示对比界面；
7. 材料、边界条件和载荷定义界面；
8. CalculiX 求解完成后的日志界面；
9. VTK 结果云图和极值标记界面；
10. CSV 或报告导出结果界面。
...
MyCAE 当前已经形成轻量级 CAE 软件的基本闭环，覆盖工程管理、几何建模、面组定义、Gmsh 网格划分、CalculiX 结构求解和 VTK 结果后处理。近期新增的网格透明显示功能进一步增强了三维查看能力，使网格展示不再只是固定样式，而是能够根据观察目的切换为实体或半透明模式。

当前系统已经具备以下阶段性成果：

- 实现了 Qt 桌面 CAE 软件基础框架；
- 打通了 Open CASCADE、Gmsh、CalculiX 和 VTK 的集成链路；
- 实现了几何、网格、工况、求解和结果显示的完整流程；
- 建立了面组与物理组机制，用于连接几何面和求解边界；
- 提供了网格显示、网格透明显示和结果云图等三维可视化能力；
- 建立了求解器插件接口，为 OpenFOAM 和其他求解器扩展预留空间；
- 提供了样例工程、UI 验证和自动截图采集能力。

后续可以继续增加以下功能：

1. 一键仿真向导  
   将几何选择、材料设置、约束设置、载荷设置、网格生成和求解运行串成一个向导流程，降低演示和使用门槛。

2. 更多载荷类型  
   增加面压力、集中力、重力等结构载荷，使结构分析场景更加完整。

3. 自动仿真报告  
   自动汇总工程名称、材料、网格数量、边界条件、载荷、最大位移、最大应力和结果截图，生成 Markdown、HTML 或 Word 报告。

4. 结果对比功能  
   对不同载荷或不同工况下的结果进行表格化对比，例如最大位移、最大应力、节点数和单元数。

5. OpenFOAM 最小 CFD 闭环  
   先实现简单流道 case 的生成、运行和 VTK 后处理，再逐步扩展复杂 CFD 功能。
